...
<{if $data.start != null}>
<p><{$data.itemcount}> item<{if $data.start <> 1}>s<{/if}> starting with <{if $data.start}><{$data.start}><{else}>numbers<{/if}></p>
...
<div style="width:170px; height:190px; border-style:solid; border-width:1px; padding:5px; margin:5px; float:left; text-align:center;">
    <a href="request.php?op=s_album&info=<{$list.linkcode}>">
        <img src="<{$data.baseurl}>/<{$data.djid}>/<{$list.cover}>" style="max-width:160px; max-height:160px;"><br/>
        <{$list.album}></a>
</div>
<{/if}>
<{if $data.op == "b_artist"}><a href="request.php?op=s_artist&info=<{$list.linkcode}>" align=left><{$list.artist}></a><br/><{/if}>
<{/foreach}>
<{/if}>
<{else}>
<h3>Last <{$data.albumlimit}> Additions</h3>
<{foreach item=list from=$data.newalbums}>
<p><a href="request.php?op=s_album&info=<{$list.linkcode}>"><img src="<{$data.baseurl}>/<{$data.djid}>/<{$list.cover}>" width=40 height=40 align=left padding=2px></a><i><{$list.album}></i><br><{$list.date_add}></p>
<{/foreach}>
<{/if}>
...
<{$data.trackcount}> tracks by <i><{$data.artist}></i>
<hr/>
...
<div style="border-style:solid; border-width:1px; padding:5px; margin:5px; overflow:hidden">
    <div style="width:170px; float:left; text-align:center">
        <img src="<{$data.baseurl}>/<{$data.djid}>/<{$album.picture}>" style="max-width:160px" onerror="uhq_nocover(this)"><br/>
