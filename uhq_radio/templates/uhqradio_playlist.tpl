...
<table>
    <tr>
        ...
        <td align=right valign=middle>
            ...
            <{else}>
            &lt;&lt; <a href="sm-tracklist.php?pl_start=<{$data.start-$data.limit}><{if $data.urlsearch}>&pl_search=<{$data.urlsearch}><{/if}>">
            <{/if}>
            ...
            <{$data.start+$data.limit}> :: <a href="sm-tracklist.php?pl_start=<{$data.start+$data.limit}><{if $data.urlsearch}>&pl_search=<{$data.urlsearch}><{/if}>"><{$smarty.const._MD_UHQRADIO_PLAYLIST_NEXT}></a> &gt;&gt;
            <{else}>
            <{$data.trackcount}>
            <{/if}>
        </td>
...
<table>
    <tr>
        <td align=left>
            <form method=POST action="sm-tracklist.php">
                Search:
                <input type="text" name="pl_search" size="30" <{if $data.rawsearch}>value="<{$data.rawsearch}>"<{/if}>>
        ...
        <td align=right>
            <form method=POST action="sm-tracklist.php">
                <select name="pl_limit" size="1" onchange="this.form.submit();">
                    ...
                    <option value="<{$list}>"
                    <{if $list == $data.limit}>selected<{/if}>><{$list}> per page</option>
                    <{/foreach}>
                </select>
