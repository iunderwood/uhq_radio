...
<{/if}>
<{if $data.op == "b_artist"}><a href="?op=s_artist&info=<{$list.linkcode}>" align=left><{$list.artist}></a><br/><{/if}>
<{/foreach}>
<{/if}>
<{else}>
...
<div style="overflow:hidden; padding:2px;">
    <a href="index.php?op=s_album&info=<{$list.linkcode}>">
        <img src="<{$data.baseurl}>/<{$data.djid}>/<{$list.cover}>" width=40 height=40 align=left padding=2px>
...
<{$data.trackcount}> <{$smarty.const._MD_UHQRADIO_INDEX_TRACKSBY}> <i><{$data.artist}></i>
<hr/>
...
<div style="border-style:solid; border-width:1px; padding:5px; margin:5px; overflow:hidden">
    ...
    <div style="margin-left:180px">
        <table>
            ...
            <td><{$track.title}></td>
            <td><a href="javascript:songinfo(<{$track.ID}>)"><{$smarty.const._MD_UHQRADIO_INDEX_INFO}></a></td>
            <td><{$track.mmss}></td>
            </tr>
            <{/foreach}>
        </table>
