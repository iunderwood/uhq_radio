...
function uhqradio_sam_opendb($djid,$host) {
	...
	$samdb = mysql_pconnect($host.":".$djinfo['rdb_port'],$djinfo['rdb_user'],$djinfo['rdb_pass']);
	...
	mysql_select_db($djinfo['rdb_name'],$samdb);
...
function uhqradio_sam_upcoming($samdb,$limit,$ids) {
	...
	$result = mysql_query($query,$samdb);
	if ($result == false) {
...
function uhqradio_sam_nowplaying($samdb) {
	...
	if ( $row = mysql_fetch_assoc($result) ) {
		if ($row['requestID']) {
			...
			$req_result = mysql_query($req_query,$samdb);
			if ($req_result) {
				if ($req_row = mysql_fetch_assoc($req_result) ) {
...
function uhqradio_sam_countpl($samdb,$where=null) {
	...
	$result = mysql_query($query,$samdb);
	if ($result == false) {
		echo "bad query: ".mysql_error($samdb)."<br>".$query;
		return false;
	} else {
...
function uhqradio_sam_displayalbums($samdb,$where=null) {
	...
	$result = mysql_query($query,$samdb);
	if ($result == false) {
		echo "bad query".mysql_error()."<br>".$query;
		// Return if query fails.
		return false;
	}
...
function uhqradio_sam_getalbum($samdb,$album) {
	...
	if ($i=0) {
		...
		if ($ac > 1) $data['album']['artist'] = "Various Artists"; else $data['album']['artist'] = utf8_encode($artist);
	}
	return $data;
}
...
function uhqradio_sam_getartist($samdb,$artist) {
	...
	$albumseq=0; $album=null; $trackcount = 0;
...
function uhqradio_sam_songinfo($samdb,$songid) {
	...
	$result = mysql_query($query,$samdb);
	if ($result == false) {
		echo "Bad Query: ".mysql_error()."<br>".$query."<br>";
		return false;
	}
...
function uhqradio_sam_djtest($djid,$host) {
	...
	if ($samdb == false) {
		...
		$data['test'][1]['error'] = mysql_errno().": ".mysql_error();
	} else {
		// Only test selection if we can open up a DB resource
		$data['test'][1]['status'] = 1;
		if (mysql_select_db($djinfo['rdb_name'],$samdb) == false) {
			// Set error message if we can't select the database
			$data['test'][2]['status'] = -1;
			$data['test'][2]['error'] = mysql_errno($samdb).": ".mysql_error($samdb);
		} else {
			$data['test'][2]['status'] = 1;
		}
	}
