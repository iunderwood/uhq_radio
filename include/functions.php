...
function uhqradio_dj_onair($chid) {
	...
	if ( ($channel['flag_djid'] == 1) && ($channel['text_mpid'] > 0) ) {
		...
		// Extract DJ ID
		$onair['djid'] = uhqradio_getinfos($cleanxml,$mountinfo['type'],$channel['flag_d_sol'],
...
function uhqradio_iceauthcheck() {
	...
	$module_object = &$module_handler->getByDirname('uhq_iceauth');
